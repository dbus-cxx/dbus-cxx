...
int main()
{
  ...
  if( conn->request_name( "dbuscxx.example.time.server", DBUSCXX_NAME_FLAG_REPLACE_EXISTING ) !=
      DBus::RequestNameResponse::PrimaryOwner )
      return 1;

  std::shared_ptr<DBus::Object> object = conn->create_object("/dbuscxx/example/Time", DBus::ThreadForCalling::DispatcherThread);
