...
int main()
{
  ...
  ret = conn->request_name( "dbuscxx.example.computer.server", DBUS_NAME_FLAG_REPLACE_EXISTING );
  if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != ret) return 1;

  Examples::Computer computer;

  DBus::Example::ComputerAdapter::pointer adapter = DBus::Example::ComputerAdapter::create(&computer);
